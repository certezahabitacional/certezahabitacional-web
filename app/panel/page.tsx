...
export default async function PanelPage({ searchParams }: { searchParams: Promise<{ zonaId?: string }> }) {
  ...
  {esDirector && reportesPendientes > 0 && <section className="mb-6 rounded-3xl border border-violet-300/25 bg-violet-300/10 p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-black uppercase tracking-widest text-violet-200">Aviso para Dirección</p><h2 className="mt-1 text-xl font-black">{reportesPendientes} reporte{reportesPendientes===1?"":"s"} esperando autorización</h2><p className="mt-1 text-sm text-slate-300">Revisa, corrige directamente si es necesario, devuelve al Inspector con observaciones o autoriza para convertirlo en reporte final.</p></div><Link href="/panel/inspecciones?estado=REPORTE_PENDIENTE" className="rounded-xl bg-violet-300 px-5 py-3 text-center text-sm font-black text-slate-950">REVISAR REPORTES PENDIENTES</Link></div></section>}
