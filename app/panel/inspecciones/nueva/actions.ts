...
import { establecerAsignacionInspeccion, usuarioAsignadoAInspeccion } from "@/lib/asignaciones-inspeccion";
...
export async function crearInspeccion(formData:FormData){
  ...
  if(inspectorId){inspectorSeleccionado=await prisma.inspector.findFirst({where:{id:inspectorId,activo:true,usuario:{activo:true,rol:RolUsuario.INSPECTOR,zonaId:zona.id}},select:{id:true,usuario:{select:{id:true,nombre:true,zonaId:true}}}});if(!inspectorSeleccionado)errorNuevaInspeccion("El Inspector seleccionado no está disponible en esta zona.",antecedenteId||undefined);}
