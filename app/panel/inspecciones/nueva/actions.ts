...
export async function crearInspeccion(formData: FormData) {
  ...
  const usuarioActual = await prisma.usuario.findUnique({
    ...
    select: { id: true, nombre: true, rol: true, activo: true, zonaId: true },
  });
  ...
  const [plantilla, zona, inmueble] = await Promise.all([
    prisma.plantillaInspeccion.findFirst({
      ...
      select: { id: true, nombre: true, codigo: true, tipoServicio: true, requiereGerenteZona: true, requiereCoordinador: true },
    }),
    prisma.zona.findFirst({
      where: { id: zonaId, activa: true },
      select: { id: true, nombre: true, codigo: true, zonaHoraria: true },
    }),
    prisma.inmueble.findUnique({
      where: { id: inmuebleId },
      select: { id: true, clienteId: true, alias: true, tipo: true, direccion: true, ciudad: true, superficieConstruccionM2: true },
    }),
  ]);
  ...
  let inspectorSeleccionado: {
    id: string;
    usuario: { id: string; nombre: string; zonaId: string | null; gerenteId: string | null; coordinadorId: string | null };
  } | null = null;

  if (inspectorId) {
    inspectorSeleccionado = await prisma.inspector.findFirst({
      where: { id: inspectorId, activo: true, usuario: { activo: true, rol: RolUsuario.INSPECTOR } },
      select: { id: true, usuario: { select: { id: true, nombre: true, zonaId: true, gerenteId: true, coordinadorId: true } } },
    });
  ...
  if (plantilla.requiereGerenteZona) {
    ...
    const gerenteValido = await prisma.usuario.findFirst({
      where: { id: inspectorSeleccionado.usuario.gerenteId, rol: RolUsuario.GERENTE, activo: true, zonaId: zona.id }, select: { id: true },
    });
  ...
  if (plantilla.requiereCoordinador) {
    ...
    const coordinadorValido = await prisma.usuario.findFirst({
      where: { id: inspectorSeleccionado.usuario.coordinadorId, rol: RolUsuario.COORDINADOR, activo: true, zonaId: zona.id }, select: { id: true },
    });
  ...
  let inspeccionAnterior: { id: string; folio: string; numeroInspeccion: number } | null = null;
  ...
  if (antecedenteId) {
    ...
    const maximo = await prisma.inspeccion.aggregate({ where: { inmuebleId }, _max: { numeroInspeccion: true } });
  ...
  const totalDelYear = await prisma.inspeccion.count({ where: { creadoEn: { gte: inicioYear, lt: inicioSiguienteYear } } });
  let consecutivo = totalDelYear + 1;
  let folio = `CH-${year}-${String(consecutivo).padStart(4, "0")}`;
  while (await prisma.inspeccion.findUnique({ where: { folio }, select: { id: true } })) {
    consecutivo += 1;
