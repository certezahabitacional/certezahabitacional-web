...
const BASE: Record<string,string[]> = {
  ARQUITECTONICO:["Distribución y correspondencia de espacios","Dimensiones y circulaciones","Muros, vanos y niveles"], FACHADAS:["Correspondencia de fachadas","Alineamientos, desplomes y remates","Sellos y encuentros exteriores"], HIDRAULICA:["Alimentación y distribución de agua","Válvulas, conexiones y fugas","Presión y funcionamiento de muebles"], SANITARIA:["Descargas y pendientes","Registros y ventilaciones","Pruebas de funcionamiento y fugas"], GAS:["Tuberías, válvulas y conexiones","Ubicación y ventilación","Prueba de hermeticidad aplicable"], ELECTRICA:["Tablero, protecciones y circuitos","Contactos, apagadores y polaridad","Canalizaciones y puesta a tierra"], PUERTAS_VENTANAS:["Alineación, plomo y nivel","Apertura, cierre y herrajes","Sellos, vidrios y acabados"], ACABADOS:["Pisos y piezas huecas","Muros, plafones y pintura","Juntas, boquillas y remates"], AIRE_ACONDICIONADO:["Equipos y ubicación","Drenajes y alimentación","Operación y distribución"], VOZ_DATOS:["Salidas y canalizaciones","Ubicación contra proyecto","Identificación y terminaciones"], OTROS:["Conceptos particulares del proyecto"]
};
...
export async function agregarItemManual(fd:FormData){const u=await actorPreparacion(),id=t(fd,"inspeccionId"),area=t(fd,"area"),concepto=t(fd,"concepto"),especificacion=t(fd,"especificacion")||null;if(!(await tablas()))volver(id,"error","La preparación técnica aún no está habilitada en esta base de datos.");if(!area||!concepto)volver(id,"error","Área y concepto son obligatorios.");await prisma.$executeRaw`INSERT INTO "GuiaInspeccionItem" ("id","inspeccionId","origen","area","concepto","especificacion","orden","creadoPorId") VALUES (${randomUUID()},${id},'MANUAL',${area},${concepto},${especificacion},9999,${u.id})`;await registrarAuditoria({tipo:TipoEvento.EDITAR,entidad:"GuiaInspeccionItem",inspeccionId:id,usuarioId:u.id,descripcion:`${u.rol} agregó manualmente a la guía: ${area} / ${concepto}.`});revalidatePath(`/panel/inspecciones/${id}/preparacion`);volver(id,"ok","Concepto agregado a la guía.")}
export async function cambiarEstadoItemGuia(fd:FormData){const u=await usuarioActual(),id=t(fd,"inspeccionId"),itemId=t(fd,"itemId"),completado=t(fd,"completado")==="true";if(!(await tablas()))volver(id,"error","La guía técnica aún no está habilitada.");const ins=await prisma.inspeccion.findUnique({where:{id},select:{estado:true,inspectorId:true}});if(!ins)volver(id,"error","La inspección no existe.");if(ins.estado==="FINALIZADA"||ins.estado==="CANCELADA")volver(id,"error","El expediente ya no admite cambios en la guía.");if(u.rol===RolUsuario.INSPECTOR){if(!u.inspector?.id||ins.inspectorId!==u.inspector.id)redirect('/acceso');if(ins.estado!=="EN_PROCESO")volver(id,"error","El Inspector completa la guía únicamente durante la inspección en campo.");}else if(u.rol!==RolUsuario.DIRECTOR&&u.rol!==RolUsuario.ADMINISTRADOR&&u.rol!==RolUsuario.GERENTE)redirect('/acceso');await prisma.$executeRaw`UPDATE "GuiaInspeccionItem" SET "completado"=${completado},"actualizadoEn"=NOW() WHERE "id"=${itemId} AND "inspeccionId"=${id}`;await registrarAuditoria({tipo:TipoEvento.EDITAR,entidad:"GuiaInspeccionItem",entidadId:itemId,inspeccionId:id,usuarioId:u.id,descripcion:`${u.rol} marcó un concepto de la guía como ${completado?'completado':'pendiente'}.`});revalidatePath(`/panel/inspecciones/${id}/preparacion`);volver(id,"ok",completado?"Concepto marcado como revisado.":"Concepto devuelto a pendiente.")}
