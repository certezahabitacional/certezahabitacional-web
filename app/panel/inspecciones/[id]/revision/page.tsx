      ...
      <div className="mx-auto max-w-5xl">
        ...
        {esDirector && pendienteRevision && (
          <section className="mt-5 rounded-3xl border border-violet-300/30 bg-violet-300/10 p-6">
            ...
            <h2 className="mt-2 text-2xl font-black">REPORTE pendiente de autorización</h2>
            <p className="mt-2 text-sm leading-6 text-slate-300">
              Abre primero el REPORTE para revisarlo completo. Después regresa a esta pantalla para autorizar y liberar el REPORTE y el CERTIFICADO, o devolverlo al Inspector con observaciones.
            </p>
        ...
        {puedeGerencia && pendienteRevision && !inspeccion.liberacionBloqueada && <section className="mt-6 grid gap-4 lg:grid-cols-2">
          <form action={aprobarGerencia} className="rounded-3xl border border-emerald-400/20 bg-slate-900 p-5"><input type="hidden" name="inspeccionId" value={id} /><h2 className="text-xl font-black text-emerald-300">{esV1 ? "Aprobar revisión previa" : "Aprobar y finalizar"}</h2><p className="mt-2 text-sm text-slate-400">{esV1 ? "Esta aprobación de Gerencia no finaliza V1. La autorización final, liquidación y certificado corresponden a Dirección." : inspeccion.requiereCoordinador ? "Requiere visto bueno vigente de Coordinación." : "Esta inspección no requiere Coordinación previa."}</p>{esV1 && inspeccion.requiereCoordinador && <p className="mt-2 text-xs font-bold text-cyan-300">Requiere visto bueno vigente de Coordinación.</p>}<textarea name="comentario" rows={3} placeholder="Comentario opcional" className="mt-4 w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3" /><button disabled={!estadoRevision.completo || (inspeccion.requiereCoordinador && !vistoBuenoCoordinador)} className="mt-3 w-full rounded-full bg-emerald-300 px-5 py-3 font-black text-slate-950 disabled:cursor-not-allowed disabled:opacity-40">{esV1 ? "Aprobar revisión previa" : "Aprobar y finalizar"}</button></form>
