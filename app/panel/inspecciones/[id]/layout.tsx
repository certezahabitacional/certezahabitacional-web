  ...
  return <>
    ...
    <nav className="print:hidden fixed bottom-4 left-1/2 z-50 w-[calc(100%-2rem)] max-w-[1500px] -translate-x-1/2 rounded-3xl border border-white/10 bg-slate-950/95 p-2 shadow-2xl backdrop-blur">
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-7">
        ...
        {esV1 ? <><Link href={`/panel/inspecciones/${id}/protocolo`} className="rounded-2xl border border-white/10 px-3 py-3 text-center text-xs font-black text-amber-300 sm:text-sm">Protocolo V1</Link><Link href={`/panel/inspecciones/${id}/areas`} className="rounded-2xl border border-white/10 px-3 py-3 text-center text-xs font-black text-emerald-300 sm:text-sm">Áreas V1</Link></> : <><Link href={`/panel/inspecciones/${id}/captura`} className="rounded-2xl border border-white/10 px-3 py-3 text-center text-xs font-black text-violet-300 sm:text-sm">Seguimiento V{inspeccion.numeroInspeccion}</Link><Link href={`/panel/inspecciones/${id}/evidencia-control`} className="rounded-2xl border border-white/10 px-3 py-3 text-center text-xs font-black text-amber-300 sm:text-sm">Evidencia 4+</Link></>}
