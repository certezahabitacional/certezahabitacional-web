  ...
  return <main className="min-h-screen bg-slate-950 px-6 py-8 text-white"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-8 space-y-5">{cotizaciones.map((c) => {
      ...
      return <details key={c.id} className="rounded-3xl border border-white/10 bg-slate-900 p-5" open={Boolean(busqueda)}>
        <summary className="cursor-pointer list-none"><div className="grid gap-3 md:grid-cols-4 lg:grid-cols-8"><Dato t="Cotización" v={c.folio}/><Dato t="Cliente" v={c.cliente.nombre}/><Dato t="Inmueble" v={c.inmueble?.alias ?? "Por definir"}/><Dato t="Versión" v={c.inspeccion ? `V${c.inspeccion.numeroInspeccion}` : "Por abrir"}/><Dato t="Importe" v={dinero(total)}/><Dato t="Pagado" v={`${dinero(pagado)} · ${porcentaje.toFixed(0)}%`}/><Dato t="Pendiente" v={dinero(pendiente)}/><Dato t="Estado" v={c.estadoPago}/></div></summary>
