    ...
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-white sm:px-6">
      <div className="mx-auto max-w-[1500px]">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-amber-300 sm:text-sm sm:tracking-[0.28em]">Base maestra de consulta</p>
            <h1 className="mt-2 text-4xl font-black">Clientes</h1>
            <p className="mt-3 max-w-4xl text-sm text-slate-400 sm:text-base">{usuarioActual.rol === RolUsuario.DIRECTOR ? "Dirección consulta todas las zonas, separadas mediante el filtro de zona." : `Consulta limitada a ${usuarioActual.zona?.nombre ?? "tu zona"}.`}</p>
          </div>

          <form className="grid gap-2 sm:grid-cols-[minmax(240px,1fr)_180px_180px_210px_auto]">
            <input name="q" defaultValue={q} placeholder="Buscar folio, nombre, teléfono, correo o domicilio" className="rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-sm sm:rounded-full sm:px-5" />
            <select name="tipo" defaultValue={tipo} className="rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-sm sm:rounded-full sm:px-5"><option value="">Todos los tipos</option>{Object.entries(etiquetas).map(([valor, etiqueta]) => <option key={valor} value={valor}>{etiqueta}</option>)}</select>
            <select name="acceso" defaultValue={acceso} className="rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-sm sm:rounded-full sm:px-5"><option value="">Todos los accesos</option><option value="ASIGNADO">Acceso asignado</option><option value="SIN_ACCESO">Sin acceso</option></select>
            {usuarioActual.rol === RolUsuario.DIRECTOR ? <select name="zonaId" defaultValue={params.zonaId ?? ""} className="rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-sm sm:rounded-full sm:px-5"><option value="">Todas las zonas</option>{zonas.map(z => <option key={z.id} value={z.id}>{z.nombre} · {z.codigo}</option>)}</select> : <div className="rounded-2xl border border-cyan-300/20 bg-cyan-300/5 px-4 py-3 text-sm font-bold text-cyan-200 sm:rounded-full sm:px-5">{usuarioActual.zona?.nombre}</div>}
            <button className="rounded-2xl bg-cyan-300 px-5 py-3 text-sm font-black text-slate-950 sm:rounded-full sm:border sm:border-white/15 sm:bg-transparent sm:text-white">Buscar / filtrar</button>
          </form>
        </div>

        <div className="mobile-card-list mt-6 sm:hidden">
          {clientes.map((cliente) => <article key={cliente.id} className="mobile-card"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-sm font-black text-cyan-300">{cliente.folio}</p><p className="mt-1 text-lg font-black">{cliente.nombre}</p></div>{cliente.usuario ? <span className={`rounded-full px-3 py-1 text-[11px] font-black ${cliente.usuario.activo ? "bg-emerald-400/10 text-emerald-300" : "bg-rose-400/10 text-rose-300"}`}>{cliente.usuario.activo ? "ACCESO" : "INACTIVO"}</span> : <span className="rounded-full bg-slate-400/10 px-3 py-1 text-[11px] font-black text-slate-400">SIN ACCESO</span>}</div><div className="mt-4 grid grid-cols-2 gap-3"><div><p className="mobile-label">Tipo</p><p className="mt-1 text-sm">{etiquetas[cliente.tipo]}</p></div><div><p className="mobile-label">Teléfono</p><p className="mt-1 text-sm">{cliente.telefono ?? "—"}</p></div></div><div className="mt-3"><p className="mobile-label">Correo</p><p className="mt-1 break-all text-sm">{cliente.correo ?? "—"}</p></div><div className="mt-3"><p className="mobile-label">Domicilio</p><p className="mt-1 text-sm text-slate-300">{[cliente.direccion, cliente.colonia, cliente.ciudad, cliente.estado].filter(Boolean).join(", ") || "—"}</p></div></article>)}
          {clientes.length===0&&<div className="mobile-card text-center text-slate-400">No hay clientes con esos filtros dentro de la zona seleccionada.</div>}
        </div>

        <div className="mt-8 hidden overflow-x-auto rounded-3xl border border-white/10 bg-slate-900/70 sm:block">
