...
export default async function PanelLayout({ children }: { children: ReactNode }) {
  ...
  const descripcionPorRol: Partial<Record<RolUsuario, string>> = {
    [RolUsuario.DIRECTOR]: "Control ejecutivo, auditoría y liberación",
    [RolUsuario.ADMINISTRADOR]: "Operación administrativa y comercial",
    [RolUsuario.GERENTE]: "Control operativo y seguimiento de zona",
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <PlatformHeader
        ...
        subtitle={`Sesión iniciada como ${nombreUsuario} · ${descripcionPorRol[usuarioActual.rol] ?? "Operación del sistema"}`}
        actions={logout}
      />
