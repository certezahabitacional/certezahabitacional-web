...
function estatusAgenda(estado: EstadoInspeccion) { if (estado === EstadoInspeccion.FINALIZADA) return "FINALIZADA"; if (estado === EstadoInspeccion.EN_PROCESO) return "EN PROCESO"; return "PROGRAMADA"; }

export default async function AgendaPage({ searchParams }: { searchParams: Promise<{ q?: string; estado?: string; ok?: string; error?: string }> }) {
  ...
  const p = await searchParams; const q = (p.q ?? "").trim(); const filtro = (p.estado ?? "").trim();

  const inspecciones = await prisma.inspeccion.findMany({
    where: { ...alcance, estado: { in: [EstadoInspeccion.PROGRAMADA, EstadoInspeccion.EN_PROCESO, EstadoInspeccion.FINALIZADA] }, ...(q ? { OR: [{ folio: { contains: q, mode: "insensitive" } }, { cliente: { nombre: { contains: q, mode: "insensitive" } } }, { inmueble: { alias: { contains: q, mode: "insensitive" } } }, { direccion: { contains: q, mode: "insensitive" } }, { cotizacion: { folio: { contains: q, mode: "insensitive" } } }] } : {}) },
    select: { id: true, folio: true, fechaProgramada: true, estado: true, direccion: true, zonaHoraria: true, cliente: { select: { nombre: true } }, inmueble: { select: { alias: true } }, cotizacion: { select: { folio: true } }, zona: { select: { zonaHoraria: true } } },
    orderBy: [{ fechaProgramada: "desc" }, { folio: "desc" }], take: 500,
  });
  const filas = inspecciones.filter(i => !filtro || i.estado === filtro);

  return <main className="min-h-screen bg-slate-950 px-4 py-8 text-white sm:px-6"><div className="mx-auto max-w-[1650px]">
    <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-sm font-black uppercase tracking-[0.28em] text-amber-300">Operación</p><h1 className="mt-2 text-4xl font-black">Agenda</h1><p className="mt-3 max-w-4xl text-slate-400">Consulta central de servicios programados. Solo Dirección y Administración pueden modificar fecha agendada o estatus.</p></div>
    <form className="grid gap-2 sm:grid-cols-[minmax(300px,1fr)_200px_auto]"><input name="q" defaultValue={q} placeholder="Buscar inspección, cotización, cliente, inmueble o domicilio" className="rounded-full border border-white/10 bg-slate-900 px-5 py-3 text-sm"/><select name="estado" defaultValue={filtro} className="rounded-full border border-white/10 bg-slate-900 px-5 py-3 text-sm"><option value="">Todos los estatus</option><option value="PROGRAMADA">Programada</option><option value="EN_PROCESO">En proceso</option><option value="FINALIZADA">Finalizada</option></select><button className="rounded-full border border-white/15 px-5 py-3 text-sm font-black">Buscar / filtrar</button></form></div>
