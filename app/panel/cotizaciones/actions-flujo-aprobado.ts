...
export async function cancelarCotizacion(formData: FormData) {
  ...
  const bloquean=[EstadoInspeccion.EN_PROCESO,EstadoInspeccion.REPORTE_PENDIENTE,EstadoInspeccion.FINALIZADA];if(c.inspeccion&&bloquean.includes(c.inspeccion.estado)) volver("error",`La inspección ${c.inspeccion.folio} ya avanzó y requiere resolución administrativa.`);
