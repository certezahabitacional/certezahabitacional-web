...
export default async function InspectoresPage({ searchParams }: { searchParams: Promise<{ q?: string; zonaId?: string; estado?: string }> }) {
  const usuarioActual = await obtenerUsuarioConAlcanceZona("/panel/inspectores");
  ...
  const zonaId = zonaEfectivaId(usuarioActual, params.zonaId);
  const zonas = rol === RolUsuario.DIRECTOR ? await prisma.zona.findMany({ where: { activa: true }, select: { id: true, nombre: true, codigo: true }, orderBy: { nombre: "asc" } }) : [];

  const alcanceJerarquico = rol === RolUsuario.GERENTE
  ...
  const inspectores = await prisma.inspector.findMany({
    ...
    select: {
      id: true, telefono: true, especialidad: true, cedula: true, ciudad: true, activo: true,
      usuario: { select: { nombre: true, email: true, zona: { select: { nombre: true, codigo: true } }, coordinador: { select: { nombre: true } }, gerente: { select: { nombre: true } } } },
    },
      ...
      <div className="mx-auto max-w-[1650px]">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-4xl text-slate-400">{rol===RolUsuario.DIRECTOR?"Dirección consulta todas las zonas con separación por filtro.":`Consulta limitada a ${usuarioActual.zona?.nombre ?? "tu zona"} y a la jerarquía de tu rol.`}</p>
          </div>
        ...
        <div className="mt-8 max-h-[72vh] overflow-auto rounded-3xl border border-white/10 bg-slate-900/70">
          <table className="min-w-[1550px] w-full border-collapse text-left">
            ...
            <tbody>{inspectores.map(i => <tr key={i.id} className="border-t border-white/5 hover:bg-white/[0.025]"><td className="px-4 py-4 font-bold">{i.usuario.nombre}</td><td className="px-4 py-4">{i.telefono ?? "—"}</td><td className="px-4 py-4">{i.usuario.email}</td><td className="px-4 py-4">{i.especialidad ?? "—"}</td><td className="px-4 py-4">{i.cedula ?? "—"}</td><td className="px-4 py-4">{i.ciudad ?? "—"}</td><td className="px-4 py-4">{i.usuario.zona ? `${i.usuario.zona.nombre} · ${i.usuario.zona.codigo}` : "—"}</td><td className="px-4 py-4">{i.usuario.coordinador?.nombre ?? "—"}</td><td className="px-4 py-4">{i.usuario.gerente?.nombre ?? "—"}</td><td className="px-4 py-4"><span className={`rounded-full px-3 py-1 text-xs font-black ${i.activo ? "bg-emerald-400/10 text-emerald-300" : "bg-slate-400/10 text-slate-400"}`}>{i.activo ? "ACTIVO" : "INACTIVO"}</span></td></tr>)}</tbody>
          </table>
