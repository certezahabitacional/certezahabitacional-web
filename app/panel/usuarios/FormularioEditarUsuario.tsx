...
type Props = { usuario: UsuarioEditable; rolesEditables: RolUsuario[]; zonas: Zona[]; gerentes?: unknown[]; coordinadores?: unknown[]; action: (formData: FormData) => void | Promise<void> };

export default function FormularioEditarUsuario({ usuario, rolesEditables, zonas, action }: Props) {
  ...
  return <form action={action} className="space-y-4">
    ...
    {esInspector && <><div className="rounded-2xl border border-cyan-300/20 bg-cyan-300/5 p-3 text-sm text-cyan-100">Gerente y Coordinador ya no se asignan al usuario; se vinculan en cada inspección.</div><div className="grid gap-4 sm:grid-cols-2"><Campo name="especialidad" label="Especialidad" type="text" defaultValue={usuario.inspector?.especialidad ?? ""}/><Campo name="cedula" label="Cédula profesional" type="text" defaultValue={usuario.inspector?.cedula ?? ""}/></div></>}
