...
export default function FormularioEditarUsuario({ usuario, rolesEditables, zonas, gerentes, coordinadores = [], action }: Props) {
  ...
  const [alcance, setAlcance] = useState<"GLOBAL" | "ZONA">(usuario.rol === RolUsuario.ADMINISTRADOR && usuario.zonaId ? "ZONA" : "GLOBAL");
  const esAdmin = rol === RolUsuario.ADMINISTRADOR; const esVendedor = rol === RolUsuario.VENDEDOR; const esGerente = rol === RolUsuario.GERENTE; const esCoordinador = rol === RolUsuario.COORDINADOR; const esInspector = rol === RolUsuario.INSPECTOR;
  ...
  return <form action={action} className="space-y-4">
    ...
    {esAdmin && <label className="block"><span className="mb-2 block text-sm font-bold text-slate-300">Alcance administrativo</span><select name="alcanceAdministrador" value={alcance} onChange={e=>{const v=e.target.value as "GLOBAL"|"ZONA";setAlcance(v);if(v==="GLOBAL")setZonaId("");}} className="w-full rounded-2xl border border-white/10 bg-slate-900 px-4 py-3"><option value="GLOBAL">Global</option><option value="ZONA">Por zona</option></select></label>}
