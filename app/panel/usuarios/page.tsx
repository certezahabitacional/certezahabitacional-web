...
type PageProps = {
  searchParams: Promise<{ ok?: string; error?: string; q?: string; rol?: string; estado?: string }>;
};
...
export default async function UsuariosPage({ searchParams }: PageProps) {
  ...
  if (!esDirector && !esAdministrador) {
    return <main className="grid min-h-screen place-items-center bg-slate-950 px-6 text-white"><div className="rounded-3xl border border-rose-400/20 bg-rose-400/5 p-8 text-center"><h1 className="text-2xl font-black text-rose-300">Acceso restringido</h1><p className="mt-3 text-slate-300">Solo Dirección y Administración pueden gestionar usuarios.</p></div></main>;
  }
  ...
  const [usuarios, zonas, gerentes, coordinadores] = await Promise.all([
    prisma.usuario.findMany({
      ...
      select: {
        ...
        inspector: { select: { telefono: true, especialidad: true, cedula: true, ciudad: true, activo: true } },
        cliente: { select: { folio: true, nombre: true } },
      },
      orderBy: [{ creadoEn: "desc" }, { nombre: "asc" }],
      take: 500,
    }),
    prisma.zona.findMany({ where: { activa: true }, select: { id: true, nombre: true }, orderBy: { nombre: "asc" } }),
    prisma.usuario.findMany({ where: { rol: RolUsuario.GERENTE, activo: true, zonaId: { not: null } }, select: { id: true, nombre: true, email: true, zonaId: true }, orderBy: { nombre: "asc" } }),
    prisma.usuario.findMany({ where: { rol: RolUsuario.COORDINADOR, activo: true, zonaId: { not: null }, gerenteId: { not: null } }, select: { id: true, nombre: true, email: true, zonaId: true, gerenteId: true }, orderBy: { nombre: "asc" } }),
  ]);

  const rolesCreables = esDirector
    ? ROLES_INTERNOS
    : [RolUsuario.VENDEDOR, RolUsuario.GERENTE, RolUsuario.COORDINADOR, RolUsuario.INSPECTOR];

  function puedeModificar(usuario: { id: string; rol: RolUsuario }) {
    if (usuario.id === administrador.id) return false;
    if (usuario.rol === RolUsuario.CLIENTE) return false;
    if (esDirector) return true;
    return usuario.rol !== RolUsuario.DIRECTOR && usuario.rol !== RolUsuario.ADMINISTRADOR;
  }

  function rolesEditables(usuario: { rol: RolUsuario }) {
    if (usuario.rol === RolUsuario.CLIENTE) return [RolUsuario.CLIENTE];
    return esDirector ? ROLES_INTERNOS : [RolUsuario.VENDEDOR, RolUsuario.GERENTE, RolUsuario.COORDINADOR, RolUsuario.INSPECTOR];
  }
  ...
  return <main className="min-h-screen bg-slate-950 px-4 py-8 text-white sm:px-6"><div className="mx-auto max-w-[1700px]">
    <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div>
      <p className="text-sm font-black uppercase tracking-[0.28em] text-amber-300">Administración de accesos</p>
      <h1 className="mt-2 text-4xl font-black">Usuarios</h1>
      <p className="mt-3 max-w-4xl text-slate-400">Único módulo para crear y mantener usuarios internos, contraseñas, zona, jerarquía y perfil de Inspector. Los clientes se originan automáticamente desde Pre-cotizaciones.</p>
    </div>
    <form className="grid gap-2 sm:grid-cols-[minmax(260px,1fr)_210px_170px_auto]"><input name="q" defaultValue={q} placeholder="Buscar nombre, correo, teléfono o ciudad" className="rounded-full border border-white/10 bg-slate-900 px-5 py-3 text-sm"/><select name="rol" defaultValue={p.rol ?? ""} className="rounded-full border border-white/10 bg-slate-900 px-5 py-3 text-sm"><option value="">Todos los roles</option>{Object.values(RolUsuario).map(r=><option key={r} value={r}>{etiquetaRol(r)}</option>)}</select><select name="estado" defaultValue={estadoFiltro} className="rounded-full border border-white/10 bg-slate-900 px-5 py-3 text-sm"><option value="">Todos</option><option value="ACTIVO">Activo</option><option value="INACTIVO">Inactivo</option></select><button className="rounded-full border border-white/15 px-5 py-3 text-sm font-black">Buscar / filtrar</button></form></div>
    ...
    <section className="mt-8 grid gap-8 xl:grid-cols-[420px_1fr]">
      <aside className="h-fit rounded-3xl border border-white/10 bg-slate-900 p-6"><h2 className="text-2xl font-black">Crear usuario</h2><p className="mt-2 text-sm leading-6 text-slate-400">Dirección administra todos los roles internos. Administración puede crear Vendedores, Gerentes, Coordinadores e Inspectores, pero nunca Director ni Administrador.</p><FormularioCrearUsuario rolesCreables={rolesCreables} zonas={zonas} gerentes={gerentes} coordinadores={coordinadores}/></aside>
