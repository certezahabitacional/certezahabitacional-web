...
function estadoCliente(estado: EstadoCotizacion) {
  return estado === EstadoCotizacion.ACEPTADA ? "ACEPTADA" : "POR ACEPTAR";
}

function estadoCerteza(estado: EstadoCotizacion) {
  return estado === EstadoCotizacion.ACEPTADA ? "POR AUTORIZAR" : "POR AUTORIZAR";
}
    ...
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-white sm:px-6">
      <div className="mx-auto max-w-[1700px]">
        ...
        <div className="mt-8 overflow-x-auto rounded-3xl border border-white/10 bg-slate-900/70">
          <table className="min-w-[1750px] w-full border-collapse text-left">
            <thead className="sticky top-0 z-20 bg-slate-900 shadow-[0_1px_0_rgba(255,255,255,0.08)]">
              <tr className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                ...
                <th className="px-4 py-4">Servicio contratado</th>
                <th className="px-4 py-4">Tipo de cliente</th>
                <th className="px-4 py-4">Nombre de cliente</th>
                <th className="px-4 py-4">Alias inmueble</th>
                <th className="px-4 py-4 text-right">M2 terreno</th>
                <th className="px-4 py-4 text-right">M2 construcción</th>
                <th className="px-4 py-4 text-right">Importe</th>
                <th className="px-4 py-4">Ver pre-cotización</th>
                <th className="px-4 py-4">Editar pre-cotización</th>
                <th className="px-4 py-4">Estatus cliente</th>
                <th className="px-4 py-4">Estatus Certeza</th>
              </tr>
            </thead>
            <tbody>
              {precotizaciones.map((c) => (
                <tr key={c.id} className="border-t border-white/5 align-top hover:bg-white/[0.025]">
                  <td className="px-4 py-4 font-mono text-xs font-black text-cyan-300">{c.folio}</td>
                  ...
                  <td className="px-4 py-4">
                    <a href={`/api/cotizaciones/${c.id}/pdf`} target="_blank" rel="noreferrer" className="text-cyan-300 underline underline-offset-4">Ver PDF</a>
                  </td>
                  <td className="px-4 py-4">
                    {gestiona ? (
                      <a href={`/panel/pre-cotizaciones/${c.id}/editar`} className="text-amber-300 underline underline-offset-4">Editar</a>
                    ) : (
                  ...
                  <td className="px-4 py-4">
                    {c.estado === EstadoCotizacion.ENVIADA && gestiona ? (
                      ...
                      <form action={autorizarCotizacionAceptada}>
                        <input type="hidden" name="id" value={c.id} />
                        <button className="rounded-xl bg-emerald-300 px-4 py-2 text-xs font-black text-slate-950">Autorizar</button>
                      </form>
                    ) : (
          ...
          {precotizaciones.length === 0 && (
            <div className="p-10 text-center text-slate-400">No hay pre-cotizaciones activas con esos filtros.</div>
          )}
        </div>
      </div>
    </main>
