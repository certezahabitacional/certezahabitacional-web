...
type FirmaEntrada = { imagen?: unknown };
...
function esImagenFirma(valor: string): boolean {
  return (
    valor === "" ||
    /^data:image\/png;base64,[A-Za-z0-9+/=]+$/.test(valor)
  );
}

function tamanoBase64Aproximado(dataUrl: string): number {
  if (!dataUrl) return 0;
...
async function obtenerAcceso(inspeccionId: string) {
  ...
  if (usuario.rol === RolUsuario.INSPECTOR) {
    puedeConsultar =
      Boolean(usuario.inspector?.id && usuario.inspector.activo) &&
  ...
  if (!puedeConsultar) {
    return {
      error: NextResponse.json(
        { error: "No tienes acceso a las firmas de esta inspección." },
        { status: 403 },
      ),
    };
  }
...
export async function GET(
  ...
  const acceso = await obtenerAcceso(id);
  ...
  const porTipo =
    new Map<TipoFirma, (typeof registros)[number]>();
  ...
  return NextResponse.json({
    ...
    motivoSoloLectura: acceso.motivoSoloLectura,
  });
}
...
export async function POST(
  ...
  if (!acceso.puedeModificar) {
    return NextResponse.json(
      { error: "No tienes autorización para modificar las firmas de esta inspección." },
      { status: 403 },
    );
  }
  ...
  try {
    body = (await request.json()) as SolicitudFirmas;
  } catch {
    return NextResponse.json(
      { error: "La solicitud de firmas no es válida." },
      { status: 400 },
    );
  }
  ...
  if (!imagenInspector || !imagenCliente) {
    return NextResponse.json(
      { error: "Debes registrar la firma del Inspector y la firma del Cliente." },
      { status: 400 },
    );
  }

  if (
    !esImagenFirma(imagenInspector) ||
    !esImagenFirma(imagenCliente)
  ) {
    return NextResponse.json(
  ...
  if (
    tamanoBase64Aproximado(imagenInspector) > TAMANO_MAXIMO_FIRMA ||
    tamanoBase64Aproximado(imagenCliente) > TAMANO_MAXIMO_FIRMA
  ) {
    return NextResponse.json(
      { error: "Cada firma debe ocupar como máximo 5 MB." },
      { status: 413 },
    );
  }

  const nombreInspector =
    acceso.inspeccion.inspector?.usuario.nombre ?? "Inspector sin asignar";
  ...
  await registrarAuditoria({
    ...
    entidad: "Inspeccion",
    entidadId: acceso.inspeccion.id,
