...
DATOS DEL INMUEBLE
Dirección: ${data.direccionInmueble}
Ciudad: ${data.ciudadInmueble}
m² de terreno: ${data.m2Terreno}
m² de construcción: ${data.m2Construccion}
Número de niveles: ${data.niveles || "No indicado"}
Número de recámaras: ${data.recamaras}
Número de baños: ${data.banos}
...
`.trim();
    ...
    const filasDesgloseHtml = cotizacion.conceptos
      .map(
        (item) => `<tr><td style="padding:8px 10px;border-bottom:1px solid #e5e7eb;">${escaparHtml(item.concepto)}</td><td style="padding:8px 10px;border-bottom:1px solid #e5e7eb;text-align:right;white-space:nowrap;">${dinero(item.importe)}</td></tr>`,
      )
      .join("");
